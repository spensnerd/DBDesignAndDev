...
-- --------------------------------------------------
-- 2a. Display the CONTACT_ID and SYSTEM_USER_ID columns in the SELECT clause from a join of
-- the CONTACT and SYSTEM_USER tables. You should make the join with the ON subclause based
-- on the CREATED_BY and SYSTEM_USER_ID columns, which are the foreign and primary key
-- respectively.
SELECT contact_id
, system_user_id
FROM contact c 
...
-- 2c. Display the CONTACT_ID and SYSTEM_USER_ID columns in the SELECT clause from a join of
-- the CONTACT and SYSTEM_USER tables. You should make the join with the ON subclause based
-- on the LAST_UPDATED_BY and SYSTEM_USER_ID columns, which are the foreign and primary key
-- respectively.
SELECT contact_id
, system_user_id
FROM contact c
...
-- --------------------------------------------------
-- Display the RENTAL_ID column from the RENTAL table, the RENTAL_ID and
-- ITEM_ID from the RENTAL_ITEM table, and ITEM_ID column from the ITEM table.
-- You should make a join from the RENTAL table to the RENTAL_ITEM table, and
-- then the ITEM table. Join the tables based on their respective primary and 
-- foreign key values.
...
ON       e.department_id = d.department_id INNER JOIN salary s
ON       e.salary_id = s.salary_id
WHERE    effective_start_date between TRUNC(SYSDATE) - 60 AND TRUNC(SYSDATE)
GROUP BY d.department_name
ORDER BY d.department_name;
