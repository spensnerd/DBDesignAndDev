...
,        CASE
           ...
           AND (TRUNC(SYSDATE) - TRUNC(i.release_date)) > 30 THEN i.release_date + 31
           ELSE  i.release_date
         END AS start_date
,        CASE
           WHEN  af.active_flag = 'N' THEN i.release_date + 30
           ELSE NULL
         END AS end_date
,        CASE
           WHEN  af.active_flag = 'N' AND (TRUNC(SYSDATE) - TRUNC(i.release_date)) > 30 
           OR af.active_flag = 'Y' AND (TRUNC(SYSDATE) - TRUNC(i.release_date)) < 31
           THEN CASE 
...
SET      rental_item_price =
          (SELECT   p.amount
           ...
           AND      rental_item.rental_id = r.rental_id
           AND      rental_item.rental_item_type = cl2.common_lookup_id
